...
echo 'export KUBERNETES_MASTER=http://host01:8080' >> ~/.bin/launch.sh
...
echo 'echo "Starting Kubernetes Proxy..."' >> ~/.bin/launch.sh
echo 'docker run -d --name=proxy --net=host --privileged gcr.io/google_containers/hyperkube:v1.2.2 /hyperkube proxy --proxy-mode=userspace --master=http://0.0.0.0:8080 --v=2' >> ~/.bin/launch.sh
echo 'echo "Kubernetes started"' >> ~/.bin/launch.sh
echo 'echo "Starting Kubernetes DNS..."' >> ~/.bin/launch.sh
echo 'kubectl -s http://host01:8080 create -f ~/kube-system.json' >> ~/.bin/launch.sh
echo 'kubectl -s http://host01:8080 create -f ~/skydns-rc.yaml' >> ~/.bin/launch.sh
echo 'kubectl -s http://host01:8080 create -f ~/skydns-svc.yaml' >> ~/.bin/launch.sh
...
curl -L https://git.io/vHc8w | bash